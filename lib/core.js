...
function scoreAll(cfg, dests, responses, weekends) {
  ...
  if (!result.top.length) result.diagnosis = diagnoseNoFit(people, cands);
  return result;
}

/**
 * When nothing fits everyone, explain why instead of leaving an empty table.
 * The most common real cause is structural: nobody shares a trip length, which
 * guarantees every candidate blocks at least the people on "the other side" —
 * so it's checked directly rather than inferred from blocker text.
 */
function diagnoseNoFit(people, cands) {
  let commonLengths = null;
  people.forEach((pp) => {
    const l = pp.p.lengths || [];
    commonLengths = commonLengths === null ? l.slice() : commonLengths.filter((x) => l.indexOf(x) > -1);
  });
  if (people.length && !(commonLengths || []).length) {
    return { reason: 'no-common-length', people: people.map((pp) => ({ name: pp.name, lengths: pp.p.lengths || [] })) };
  }
  if (!cands.length) return { reason: 'no-candidates' };

  const minBlocked = Math.min.apply(null, cands.map((c) => c.blockedCount));
  const tally = {};
  cands.forEach((c) => {
    if (c.blockedCount !== minBlocked) return;
    c.people.forEach((p) => p.blockers.forEach((b) => { tally[b] = (tally[b] || 0) + 1; }));
  });
  const sampleReason = Object.keys(tally).sort((a, b) => tally[b] - tally[a])[0] || '';
  return { reason: 'closest-blocked', minBlocked: minBlocked, sampleReason: sampleReason };
}
